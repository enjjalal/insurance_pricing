...
class InsuranceExperimentTracker:
    ...
    def run_baseline_experiments(self):
        ...
        results.append(self.evaluate_model(xgb_model, "XGBoost", 
                                        self.X_train, self.X_test, 
                                        self.y_train, self.y_test))
        ...
        results.append(self.evaluate_model(lgb_model, "LightGBM", 
                                        self.X_train, self.X_test, 
                                        self.y_train, self.y_test))
        
        # 8. SVR (with scaled data)
    ...
    def run_hyperparameter_tuning(self):
        ...
        with mlflow.start_run(run_name="XGBoost_Tuned"):
            xgb_tuned.fit(self.X_train, self.y_train)
            mlflow.log_params(xgb_tuned.best_params_)
            
            y_pred = xgb_tuned.predict(self.X_test)
            mae = mean_absolute_error(self.y_test, y_pred)
            r2 = r2_score(self.y_test, y_pred)
            
            mlflow.log_metric("mae", mae)
            mlflow.log_metric("r2", r2)
            mlflow.sklearn.log_model(xgb_tuned.best_estimator_, "xgb_tuned_model")
